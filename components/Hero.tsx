        ...
        <div className="hero-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 60, alignItems: 'center' }}>
          ...
          <div>
            <h1 style={{
              ...
              lineHeight: 1.0, letterSpacing: -2, color: 'var(--text)', marginBottom: 24,
            ...
            <p style={{ fontSize: 17, fontWeight: 400, color: 'var(--text-3)', lineHeight: 1.75, maxWidth: 480, marginBottom: 36, animation: 'fadeUp 0.6s 0.1s ease both' }}>
              ...
              <strong style={{ color: 'var(--text-2)', fontWeight: 600 }}>Running 24/7, without your team lifting a finger.</strong>
            </p>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 0, borderTop: '1px solid var(--border)', paddingTop: 32, animation: 'fadeUp 0.6s 0.3s ease both' }}>
              {[{ n: '10X', l: 'Faster leads' }, { n: '80%', l: 'Time saved' }, { n: '24/7', l: 'AI agents' }, { n: '6+', l: 'Platforms' }].map((s, i) => (
                <div key={i} style={{ paddingRight: 16, borderRight: i < 3 ? '1px solid var(--border)' : 'none', paddingLeft: i > 0 ? 16 : 0 }}>
                  <div style={{ fontFamily: 'EquitanSans, sans-serif', fontWeight: 900, fontSize: 28, color: 'var(--orange)', lineHeight: 1, letterSpacing: -1 }}>{s.n}</div>
                  <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-5)', marginTop: 4, letterSpacing: 0.5 }}>{s.l}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <div style={{
              ...
              boxShadow: '0 24px 80px rgba(0,0,0,0.3), inset 0 1px 0 var(--border-2)',
            }}>
              <div style={{ background: 'var(--surface-2)', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 8, borderBottom: '1px solid var(--border)' }}>
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ff5f57' }} />
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#ffbd2e' }} />
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: '#28ca41' }} />
                <div style={{ flex: 1, background: 'var(--surface-2)', borderRadius: 6, padding: '4px 12px', marginLeft: 8 }}>
                  <span style={{ fontSize: 11, color: 'var(--text-5)', fontFamily: 'monospace' }}>app.agentflow.ai / {TABS[displayed].tag.toLowerCase().replace(' ', '-')}</span>
                </div>
              </div>
            ...
            <div style={{ marginTop: 20, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 11, color: 'var(--text-5)', fontWeight: 500, letterSpacing: 0.5 }}>INTEGRATES WITH</span>
              {TRUSTED.map(t => (
                <span key={t} style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-4)', background: 'var(--surface)', padding: '4px 10px', borderRadius: 6, border: '1px solid var(--border)', letterSpacing: 0.3 }}>{t}</span>
              ))}
            </div>
          </div>
        </div>
